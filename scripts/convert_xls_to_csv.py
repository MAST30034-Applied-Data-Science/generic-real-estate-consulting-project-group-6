...
vic_income.to_csv("/root/generic-real-estate-consulting-project-group-6/data/raw/external_data/income_SA2_vic.csv")


sheet_names = ["Vic.", "Greater Melbourne", "Rest of Vic."]
path_names = ["vic", "greater_melbourne", "rest_vic"]
for i in range(0, 3):
    popu_pred = pd.read_excel("/root/generic-real-estate-consulting-project-group-6/data/raw/external_data/population_pred.xls", sheet_name=sheet_names[i], skiprows=lambda x : x not in [5] + list(range(41, 50)) + list(range(52,56)))
...
sa_popu.to_csv("/root/generic-real-estate-consulting-project-group-6/data/raw/external_data/population.csv")
